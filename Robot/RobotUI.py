...
class RobotUI(tk.Tk):
    def __init__(self, robot, navegador):
        super().__init__()
        self.robot = robot
        ...
        self.label_basura_no_reciclable = tk.Label(self, text="Basura no reciclable: {}".format(self.robot.basura_no_reciclable))
        self.label_basura_no_reciclable.pack()
        ...
        self.button_seguir_camino = tk.Button(self, text="Seguir Camino", command=self.seguir_camino)
    ...
    def seguir_camino(self):
        mensaje = self.navegador.seguir_camino()
